...
function generateContextMenu(event) {
    ...
    if (draggableElement) {
        let draggable = Whiteboard.layoutNodeRegistry[Whiteboard.getDraggableIndex(draggableElement)]
        Whiteboard.currentNode = draggable;
        ...
        if (event.target.classList.contains("path-point")) {
            ...
            if (Whiteboard.editingMode) {
                ...
                generateContextMenuButton(container, "send to front", () => { Whiteboard.logChange(); draggable.setLayer(Whiteboard.layoutNodeRegistry.length - 1) });
